...
export function useMatchFilters(predictions: PredictionResult[], results: any[]) {
    ...
    const filteredPredictions = useMemo(() => {
        return enrichedPredictions
            ...
            .sort((a, b) => activeTab === 'today' || activeTab === 'future' ? new Date(a.startTime).getTime() - new Date(b.startTime).getTime() : new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
